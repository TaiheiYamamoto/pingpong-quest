...
export default function SessionRunner({ demand }: { demand: Demand }) {
  const { push } = useToast();
    ...
    <div>
      ...
      <div className="mt-4">
        {steps[current] === "listen_and_repeat" && (
          <ListenAndRepeat genre={genre} level={level} phrases={phrases} loading={loading} />
        )}
        {steps[current] === "roleplay_ai" && <RoleplayBlock level={level} genre={genre} />}
...
function RoleplayBlock({ genre, level }: { genre: Genre; level: CEFR }) {
  const { push } = useToast();
  const audioRef = React.useRef<HTMLAudioElement | null>(null);
